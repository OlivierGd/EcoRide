<?php
// Début du fichier _card_trip.php

$trip = $item['trip'];
$booking = $item['booking'];
$role = $item['role'];
$status = $trip->getTripStatus();
$driver = \Olivierguissard\EcoRide\Model\Users::findUser($trip->getDriverId());
$car = \Olivierguissard\EcoRide\Model\Car::findCarById($trip->getVehicleId());

// Logique des actions possibles (votre code existant)
$canStartTrip = false;
$canEndTrip = false;
    ...
    <div class="card shadow-sm border-0 mb-3">
        <div class="card-body">
            ...
            <div class="d-flex gap-2 flex-wrap">
                ...
                <!-- POINT CLÉ : Bouton pour voir le trajet qui ouvre la modale -->
                <!-- IMPORTANT : L'ID de la modale doit correspondre à celui généré dans renderTripModal() -->
                <button type="button" class="btn btn-outline-primary btn-sm"
                        data-bs-toggle="modal" data-bs-target="#tripModal-<?= $trip->getTripId(); ?>">
                    <i class="bi bi-eye"></i> Voir trajet
                </button>
            </div>
        </div>
    </div>
...
<?php
// Générer la modale seulement si nous avons toutes les données nécessaires
// Évite les erreurs et les modales vides
if (function_exists('renderTripModal') && $driver && $car) {
    renderTripModal($trip, $driver, $car);
}
?>